...
@Component({
  selector: 'app-triagechart',
  templateUrl: './triagechart.component.html',
  styleUrls: ['./triagechart.component.css']
})
export class TriagechartComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    .subscribe(res=>{
      ...
      let Tri= res.map(res=>res.Triage)
...
let deadcount=0;
let expectant=[];
let expectantcount=0;


Tri.forEach((res)=>{

  if(res.includes("Immediate")){
    immediatecount= immediatecount+1;


  }
  if(res.includes("Urgent")){
    urgentcount=urgentcount+1;
  }
  if(res.includes("Delayed")){
    delaycount=delaycount+1;
  }
  if(res.includes("Expectant")){
    expectantcount=expectantcount+1;
  }
  if(res.includes("Dead")){
    deadcount=deadcount+1;
  }
})
immediate.push(immediatecount);
delay.push(delaycount);
expectant.push(expectantcount);
dead.push(deadcount);
...
//Testing cases to remove a string from a number to plot the number only.
        ...
        this.chart = new Chart('canvas',{
          ...
          type:'bar',
          data:{
            labels:['Immediate',"Urgent","Delay","Expectant","Dead" ],
            ...
            datasets:[

              {
                ...
                label:"Triage Catergory in Year 2022",

                data:[immediate,urgent,delay,expectant,dead],

                backgroundColor:['red','rgb(224, 221, 15)','green','blue','black'
              ],
